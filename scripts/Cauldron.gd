...
func _on_player_interacted(player_id, interactable_id, item, element):
	...
	if interactable_is_correct and $Timer.is_stopped() and output_item.item_id == Globals.ItemID.NONE:
	...
	if interactable_is_correct and $Timer.is_stopped() and output_item.item_id == Globals.ItemID.POTION:
		$"../Primary Light".enabled = false
